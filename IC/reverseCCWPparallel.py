...
if __name__ == "__main__":
    ...
    for e in frange(1,1.5,.5):
        ...
        for r in frange(1.,1.1,.2):
            ...
            time2map = time.time()
            print('Start mapping...')
            result = pool2algo.map(mapReverseCCWP, list(range(R))) # result is [(scores1, L1), (scores2, L2), ...]
            ...
            while coverage < tsize:
                maxk, maxv = max(iter(scores_copied.items()), key = operator.itemgetter(1))
                S.append(maxk)
                scores_copied.pop(maxk)
                T = pool2average.map(mapAvgSize, [S]*4)
                coverage = sum(T)/len(T)
    ...
    print('e_results:', sorted(iter(e_results.items()), key = lambda dk_dv1: dk_dv1[0]))
